...
const features = [
  ...
  { title: 'Market & Crypto Coverage', description: 'Separate specialized reports for traditional markets and cryptocurrency, each with domain-specific analysis.', icon: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125" /></svg> },
];
...
const testimonials = [
  { quote: 'ScraperAgent saves me 2 hours every morning. Instead of scrolling through dozens of expert accounts, I get a synthesized view with actionable signals before the market opens.', attribution: 'Day Trader', role: 'Dublin, Ireland' },
  { quote: 'The sentiment analysis is remarkably accurate. I was skeptical about AI-generated signals, but the expert attribution and source linking gives me confidence in the analysis.', attribution: 'Portfolio Manager', role: 'London, UK' },
  { quote: 'I use the crypto reports to track DeFi sentiment across multiple experts I would never have time to follow individually. The sector breakdowns are especially valuable.', attribution: 'Crypto Investor', role: 'Berlin, Germany' },
];
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16 text-center relative">
          ...
          <p className="mt-6 text-lg text-text-secondary max-w-2xl mx-auto">
            Every morning, our AI reads 40+ market experts on X, synthesizes 500+ tweets,
            and delivers actionable intelligence straight to your inbox.
          </p>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 text-xs font-mono">
              {previewSignals.map((s) => (
                <span key={s.Ticker} className="px-3 py-1.5 rounded-lg bg-surface/60 border border-border-subtle text-text-secondary">
                  <span className="font-bold text-text-primary">{s.Ticker}</span>{' '}
                  <span className={s.Direction >= 3 ? 'text-green-500' : s.Direction <= 1 ? 'text-red-500' : 'text-yellow-500'}>
      ...
      <SectionWrapper id="how-it-works" title="From Expert Tweets to Your Inbox in 4 Steps" subtitle="Our pipeline runs daily, scraping, analyzing, and synthesizing market intelligence automatically.">
        ...
        <div className="text-center mt-8">
          <Link href="/how-it-works" className="text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">
      ...
      <SectionWrapper id="experts" title="Powered by 40+ Expert Voices" subtitle="We track the most influential market commentators on X, from macro strategists to options flow analysts." variant="accent">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {report.Analysis.ExpertSentiments.map((expert) => (
            <ExpertCard key={expert.ExpertHandle} expert={expert} />
          ))}
        </div>
        <div className="flex flex-wrap items-center justify-center gap-6 text-sm text-text-muted">
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center text-xs font-bold">40+</span>
            Experts Tracked
          </div>
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-purple-500/10 text-purple-400 flex items-center justify-center text-xs font-bold">2</span>
            Domains
          </div>
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-green-500/10 text-green-400 flex items-center justify-center text-xs font-bold">500+</span>
            Tweets Daily
          </div>
        </div>
      </SectionWrapper>
      ...
      <SectionWrapper id="sectors" title="Market Sectors at a Glance" subtitle="Identify where momentum is building and where caution is warranted.">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {report.Analysis.SectorBreakdown.map((sector) => (
      ...
      <SectionWrapper id="testimonials" title="What Subscribers Are Saying" subtitle="Early adopters are already saving hours every day with ScraperAgent." variant="accent">
        ...
        <p className="text-center mt-8 text-sm text-text-muted">Join early adopters receiving daily market intelligence</p>
      </SectionWrapper>
